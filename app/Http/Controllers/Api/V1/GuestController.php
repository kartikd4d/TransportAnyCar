...
class GuestController extends ResponseController
{
    ...
    public function getVehicle(Request $request) {
        ...
        // Check if $term is empty before proceeding
        if (!empty($term)) {
            if($type == 'make') {
                $makes = Vehicle::where('make', 'LIKE', "%{$term}%")->distinct()->pluck('make');
                $data = $makes;
            } elseif ($type == 'model') {
                $pre_make = $request->input('preMake');
        ...
        if(count($data) > 0) {
            ...
            $this->sendResponse(200,_('api.succ'), $data);
        }
    }
